...
import { Copy, Tag, Calendar, Container, HardDrive, Hash, Play } from "lucide-react"
import {
  Sheet,
  SheetContent,
  SheetTitle,
  ...
  const handleScan = async (imageRef: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Job created for ${imageRef}`,
      })
    } catch (err: any) {
      console.error("Analysis Failed:", err)
      toast({
        title: "Failed to start analysis",
        description: err.message || "An error occurred while starting the analysis.",
        variant: "destructive",
      })
    } finally {
              ...
              <div className="flex items-center gap-3 w-full">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <SheetDescription className="sr-only">
                    View detailed information about the selected container image including tags, metadata, and analysis options.
                  </SheetDescription>
                </div>
                {item?.registry && (
                        ...
                        <div key={tag.name} className="group relative flex flex-col gap-2  border p-3 hover:bg-muted/30 transition-colors">
                          ...
                          <div className="flex items-center justify-between text-xs text-muted-foreground">
                            <div className="flex items-center gap-4">
                              <span className="flex items-center gap-1 max-w-lg">
